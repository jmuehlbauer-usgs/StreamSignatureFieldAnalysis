...
neo1$logP <- round(ifelse(neo1$PostP == 1, ifelse(neo1$DistP == 0, -3.5, log(neo1$DistP)), 0), 4)
neo1$DistF <- ifelse(neo1$PostF == 1, neo1$Dist - neo1$Forest, 0)
neo1$logF <- round(ifelse(neo1$PostF == 1, ifelse(neo1$DistF == 0, -3.5, log(neo1$DistF)), 0), 4)
neo1$DistW <- ifelse(neo1$PostW == 1, neo1$Dist - neo1$Wall, 0)
neo1$logW <- round(ifelse(neo1$PostW == 1, ifelse(neo1$DistW == 0, -3.5, log(neo1$DistW)), 0), 4)
...
## Try a different slope to account for breaks
P4 <- lmer(log(AbBiom) ~ logD2 + (logD2 | SiteBank), data = neo1, REML = FALSE)
P5 <- lmer(log(AbBiom) ~ logD3 + (logD3 | SiteBank), data = neo1, REML = FALSE)
P6 <- lmer(log(AbBiom) ~ logD4 + (logD4 | SiteBank), data = neo1, REML = FALSE)
P7 <- lmer(log(AbBiom) ~ logD5 + (logD5 | SiteBank), data = neo1, REML = FALSE)
P8 <- lmer(log(AbBiom) ~ logD6 + (logD6 | SiteBank), data = neo1, REML = FALSE)
P9 <- lmer(log(AbBiom) ~ logD7 + (logD7 | SiteBank), data = neo1, REML = FALSE)
	AIC4 <- AIC(P3, P4, P5, P6, P7, P8, P9)
	## All of these are at least slightly better than just distance, but some are nonsensical without other information.

## Try combined slopes to account for breaks
P10 <- lmer(log(AbBiom) ~ logD2 + logP + (logD2 | SiteBank), data = neo1, REML = FALSE)
P11 <- lmer(log(AbBiom) ~ logD3 + logW + (logD3 | SiteBank), data = neo1, REML = FALSE)
P12 <- lmer(log(AbBiom) ~ logD4 + logP + (logD4 | SiteBank), data = neo1, REML = FALSE)
P13 <- lmer(log(AbBiom) ~ logD4 + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P14 <- lmer(log(AbBiom) ~ logD4 + logP + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P15 <- lmer(log(AbBiom) ~ logD4 + logA + (logD4 | SiteBank), data = neo1, REML = FALSE)
	AIC5 <- AIC(P4, P5, P6, P10, P11, P12, P13, P14, P15)
	## Best option is 0 after any break, with a new slope (logW) after a Wall break only.

## Add intercepts for breaks
P16 <- lmer(log(AbBiom) ~ PostP + logD4 + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P17 <- lmer(log(AbBiom) ~ PostW + logD4 + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P18 <- lmer(log(AbBiom) ~ PostP + PostW + logD4 + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P19 <- lmer(log(AbBiom) ~ PostP * PostW + logD4 + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
	AIC6 <- AIC(P13, P16, P17, P18, P19)
	## Adding a pre_Forest intercept is the only thing that improves the model, and even then not by much.

## Add dummy intercepts for pre-Forest breaks
P4 <- lmer(log(AbBiom) ~ PostP + logD + (logD | SiteBank), data = neo1, REML = FALSE)
P5 <- lmer(log(AbBiom) ~ PostP + logD + (logD + PostP| SiteBank), data = neo1, REML = FALSE)
P6 <- lmer(log(AbBiom) ~ logD2 + logP + (logD2 | SiteBank), data = neo1, REML = FALSE)
P7 <- lmer(log(AbBiom) ~ logD2 + logP + (logP | SiteBank), data = neo1, REML = FALSE)
P8 <- lmer(log(AbBiom) ~ logD2 + logP + (logD2 + logP | SiteBank), data = neo1, REML = FALSE)
P9 <- lmer(log(AbBiom) ~ PostP + logD2 + logP + (logD2 | SiteBank), data = neo1, REML = FALSE)
P10 <- lmer(log(AbBiom) ~ PostP + logD2 + logP + (logD2 + logP | SiteBank), data = neo1, REML = FALSE)
...
P12 <- suppressWarnings(lmer(log(AbBiom) ~ PostW + logD + (logD + PostW| SiteBank), data = neo1, REML = FALSE))
	## Does not converge.
P13 <- lmer(log(AbBiom) ~ AnteW:logD4 + PostW:logW + (AnteW:logD4 | SiteBank), data = neo1, REML = FALSE)
P14 <- lmer(log(AbBiom) ~ AnteW:logD4 + PostW:logW + (PostW:logW | SiteBank), data = neo1, REML = FALSE)
P15 <- lmer(log(AbBiom) ~ AnteW:logD4 + PostW:logW + (AnteW:logD4 + PostW:logW | SiteBank), data = neo1, REML = FALSE)
P16 <- lmer(log(AbBiom) ~ PostW + logD2 + logW + (logD2 | SiteBank), data = neo1, REML = FALSE)
P17 <- suppressWarnings(lmer(log(AbBiom) ~ PostW + AnteW:logD4 + PostW:logW + (AnteW:logD4 + PostW:logW | SiteBank), data = neo1, REML = FALSE))
	## Does not converge.
	AIC5 <- AIC(P3, P11, P12, P13, P14, P15, P16, P17)
	## Pattern of results basically the same as for pre-Forest.

## Combine pre-Forest and Wall slopes and intercepts into a single model
P18 <- lmer(log(AbBiom) ~ PostP + PostW + logD4 + logP + logW + (logD4 | SiteBank), data = neo1, REML = FALSE)
P18 <- lmer(log(AbBiom) ~ PostP + PostW + logD5 + (logD5 | SiteBank), data = neo1, REML = FALSE)
	AIC6 <- AIC(P9, P16, P18)
...
for(i in 1:length(maxs)){
	...
	t1 <- as.data.frame(seq(0, 10000, .1))
		...
		t1$Slp <- as.numeric(rep(coefs[4], dim(t1)[1]))
		t1$logD5 <- ifelse(t1$PostPreF == 1 | t1$PostW ==1, 0, ifelse(t1$Dist == 0, -3.5, log(t1$Dist)))
		t1$logFitAB <- with(t1, Int + FInt * PostPreF + WInt * PostWall + Slp * logD5)
